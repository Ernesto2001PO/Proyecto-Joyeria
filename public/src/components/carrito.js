...
function aumentarCantidad(item_id) {
    const itemElement = document.querySelector(`.carrito-item[data-item-id="${item_id}"]`);
...
function actualizarCantidadEnServidor(item_id, cantidad) {
    ...
    .then(data => {
        ...
        cargarCarrito(); // Vuelve a cargar el carrito para obtener los datos actualizados
    })
    .catch(error => {
